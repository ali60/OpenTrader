...
Cl_macd_optimizer::Cl_macd_optimizer()
{
	...
	strcpy(kernel_name_, "macd.cl");
	strcpy(kernel_function_name_, "macd_kernel");
...
int Cl_macd_optimizer::CreateAndInitBuffers()
{
	cl_uint maCount = 40;
	...
	for (cl_uint i = 0; i < maCount; ++i)
		for (cl_uint j = 0; j < array_size_; ++j)
		{
			...
			input_ind_array_[idx] = static_cast<int>(indicator_->GetMa(j, i + 1, PRICE_OPEN)*10000.00);
		}

	test_option_count_ = 0;
	for (cl_uint i = 1; i < maCount; i++)
		for (cl_uint j = i + 1; j < maCount; j++)
			for (cl_uint k = 1; k <= 20; k++)
			{
				cl_int item = MAKELONG(k,MAKEWORD(i, j));
...
*/
int  Cl_macd_optimizer::CreateBufferArguments()
{
...
*/
cl_uint Cl_macd_optimizer::SetKernelArguments()
{
	cl_int err = CL_SUCCESS;
	int index = 0;
...
*/
long Cl_macd_optimizer::ReadAndVerify(tradeParameters* optResult)
{
	cl_int err = CL_SUCCESS;
	bool ret = true;
...
//	err = clEnqueueUnmapMemObject(command_queue_, mem_result_, resultPtr, 0, NULL, NULL);
	...
	for (cl_int i = 0; i < test_option_count_; ++i)
	{
		...
		int ma2 = ((ma_pair_list[i] >> 16) & 0xFF);
		int ma1 = ((ma_pair_list[i] >> 24) & 0xFF);
		LogInfo("MACD -profit [%d,%d,%d]=%d\n", ma1, ma2, slowing, temp);
		if (best < temp) {
